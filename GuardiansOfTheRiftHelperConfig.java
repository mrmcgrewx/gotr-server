...
public interface GuardiansOfTheRiftHelperConfig extends Config {
    ...
    @ConfigSection(
            name = "SimpleServer Settings",
            closedByDefault = true,
            position = 6,
            description = "Settings for the http server"
    )
    String simpleServerSettings = "simpleServerSettings";
    ...
    @ConfigItem(
            keyName = "bindAddress",
            name = "Bind address",
            description = "HTTP server bind address (keep 127.0.0.1 for local-only)",
            position = 0,
            section = simpleServerSettings
    )
    default String bindAddress() { return "127.0.0.1"; }
